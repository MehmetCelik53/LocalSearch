...
def generate_search_query_with_structured_output(
    ...
    """Helper function to generate search queries either tool calling or JSON mode.
    Args:
        configurable: Configuration object
        messages: List of messages to send to LLM
        tool_class: Tool class for tool calling mode
        fallback_query_field: Fallback search query if extraction fails
        tool_query_field: Field name in tool args contatining the query
...
def web_research(state: SummaryState, config: RunnableConfig):
    ...
    ducduckgo, or searxng) and formats the result for futher processing.
    ...
    Returns:
        Dictionary with state update, including sources_gathered, research_loop_count, and web research_Results
...
def summarize_sources(state: SummaryState, config: RunnableConfig):
    ...
    Returns:
        Dictionary with state update, including running_Summary key containig the udpated summary
...
def reflect_on_summary(state: SummaryState, config: RunnableConfig):
    ...
    Analyzes the current summary to identify areas for futher research and generates a new search query to address those gaps.
    Uses structured output to extract the follow-up query in JSON format..
    ...
    Returns:
        Dictionary with state update, including search_query key containing the generated
    """
    # Generate a query
    configurable = Configuration.from_runnable_config(config)
...
builder.add_edge("reflect_on_summary", route_research)
